...
export default function AccessRequests() {
  ...
  const openConfirmModal = (request: AccessRequest, isGranting: boolean): void => {
    ...
    const message = isGranting
      ? `Are you sure you want to grant access to your credential for ${request.verifierName}? They will be able to view your credential details and verification status.`
      : `Are you sure you want to deny the access request from ${request.verifierName}? They will not be able to access your credential.`;
    ...
    modals.open({
      ...
      children: (
        <>
          <Text size="sm" mb="lg">
            {message.split(request.verifierName)[0]}
            <Text span fw={600} c={textColor}>
              {request.verifierName}
            </Text>
            {message.split(request.verifierName)[1]}
          </Text>
    ...
    >
      ...
      {request.status === 'pending' && (
        <Box mt="md">
          <Text size="xs" c="dimmed" mb="xs">
            This verifier is requesting access to view your credential credentials.
          </Text>
      ...
      {request.status === 'granted' && (
        <Text size="xs" c="dimmed" mt="md">
          You've granted access to this credential. The verifier can now view your verified
          credential.
        </Text>
      )}
      ...
      {request.status === 'denied' && (
        <Text size="xs" c="dimmed" mt="md">
          You've denied access to this credential. The verifier cannot view your credential.
        </Text>
      )}
    </Card>
  );
    ...
    <Paper withBorder p="xl" radius="md" ta="center">
      ...
      <Text size="sm" c="dimmed" maw={400} mx="auto" mb="xl">
        {activeTab !== 'all'
          ...
          : "You don't have any access requests yet. When an verifier requests access to your credentials, it will appear here."}
      </Text>
    </Paper>
  );
      ...
      <Paper p="md" withBorder radius="md" mb="xl">
        <Group>
          ...
          <div>
            ...
            <Text size="xs" c="dimmed">
              When verifiers want to view your verified credentials, they send an access request.
