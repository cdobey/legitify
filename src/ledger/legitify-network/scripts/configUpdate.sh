#!/bin/bash
#
# Script to create and sign a configuration update
#
...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${TEST_NETWORK_HOME}/channel-artifacts/config_block.pb -o 176.34.66.195:7050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
...
# Function to collect signatures from all orgs
collectAllSignatures() {
  CONFIG_UPDATE_FILE=$1
  for ORG in 1 2 3; do
    setGlobals $ORG
    infoln "Signing config update as Org${ORG}"
    signConfigtxAsPeerOrg $ORG $CONFIG_UPDATE_FILE
    res=$?
