...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"legitifyChannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c legitifyChannel -s couchdb "
    println "   network.sh up createChannel -bft -c legitifyChannel -s couchdb "
